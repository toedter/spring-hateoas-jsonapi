...
class MovieModelAssembler {
  ...
  public RepresentationModel<?> toJsonApiModel(Movie movie, String[] fieldsMovies) {
    Link selfLink =
        linkTo(methodOn(MovieController.class).findOne(movie.getId(), null, null)).withSelfRel();
    ...
    // Link templatedMoviesLink = Link.of(moviesLink.getHref() +
    // "{?page[number],page[size]}").withRel("movies");
    ...
    final Affordance updatePartiallyAffordance =
        afford(
            methodOn(MovieController.class)
                .updateMoviePartially(EntityModel.of(movie), movie.getId()));
    ...
    JsonApiModelBuilder builder =
        jsonApiModel()
            .model(movie)
            .link(
                selfLink.andAffordance(updatePartiallyAffordance).andAffordance(deleteAffordance));
  ...
  public RepresentationModel<?> directorsToJsonApiModel(Movie movie) {
    Link selfLink =
        linkTo(methodOn(MovieController.class).findDirectors(movie.getId())).withSelfRel();
  ...
  public RepresentationModel<?> directorsRelationshipToJsonApiModel(Movie movie) {
    Link selfLink =
        linkTo(methodOn(MovieController.class).findDirectorsRelationship(movie.getId()))
            .withSelfRel();
    Link relatedLink =
        linkTo(methodOn(MovieController.class).findDirectors(movie.getId())).withRel("related");
