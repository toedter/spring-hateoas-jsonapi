...
class MovieLoader {
  ...
  CommandLineRunner init(MovieRepository movieRepository, DirectorRepository directorRepository) {
    return args -> {
      ...
      ClassPathResource resource = new ClassPathResource("static/movie-data/movies-250.json");

      moviesJson = readFile(resource);
  ...
  private String readFile(ClassPathResource resource) throws IOException {
