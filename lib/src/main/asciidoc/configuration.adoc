...
| MapperCustomizer | A lambda expression (`UnaryOperator<JsonMapper.Builder>`) to add additional configuration to the Jackson 3 `JsonMapper.Builder` used for serialization and deserialization. This allows customizing Jackson features, modules, or other settings. See <<mapper-customization>>. | not set
[[links-configuration]]
...
Starting with version 2.2.0, this library uses *Jackson 3* (package `tools.jackson.*` instead of `com.fasterxml.jackson.*`).
