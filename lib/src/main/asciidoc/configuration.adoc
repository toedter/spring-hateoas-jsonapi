...
see https://jsonapi.org/format/#auto-id--link-objects. To serialize the Spring HATEOAS properties title, type, and hreflang both as top-level link properties and in the meta section, set this property to `false`. See also <<links>>.
