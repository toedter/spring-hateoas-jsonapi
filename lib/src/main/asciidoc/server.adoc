...
However, you can configure this behavior using the `LinksAsResourceLevelLinks` configuration option
...
// Default configuration - links at document level
...
To place links at the resource level instead, configure `LinksAsResourceLevelLinks` to `true`:
...
// Configure links at resource level
JsonApiConfiguration config = new JsonApiConfiguration()
    .withLinksAsResourceLevelLinks(true);
----
...
These methods are particularly useful when you want to replace existing relationship data with null or empty array,
or when you need to explicitly indicate the type of empty relationship (to-one vs to-many) for your JSON:API consumers.
The methods also preserve any existing links and meta information in the relationship.
