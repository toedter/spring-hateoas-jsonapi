...
As you see, the properties title, type, and hreflang appear both as top-level link properties
as well as properties in the meta section. So the new format reflects the new JSON:API 1.1 link structure,
but stays backward compatible to version 1.x.x of this library.

If you start a new project without any existing clients, you could specify a special configuration (see <<links-configuration>>), that removes the attributes from the meta section.
...
https://docs.spring.io/spring-hateoas/docs/current-SNAPSHOT/reference/html/#server.affordances[affordances].
