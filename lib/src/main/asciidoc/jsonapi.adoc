...
JSON:API specific representation model, a paged model with automatically created paging links:
...
* if page metadata should be automatically created for a `PagedModel`, the default is `true`
* if pagination links should be automatically created for a `PagedModel`, the default is `true`
** precondition is that the paged model has a self link
** the request parameter names for page number and page size can also be configured
*** the defaults are `page[number]` and `page[size]`, which is recommended by the JSON:API specification.
