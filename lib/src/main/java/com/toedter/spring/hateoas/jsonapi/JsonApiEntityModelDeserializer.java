...
class JsonApiEntityModelDeserializer
    ...
    if (resources.size() == 1) {
      ...
      if (relationships != null) {
        ...
        for (Field field : declaredFields) {
          ...
          JsonApiRelationships relationshipsAnnotation = field.getAnnotation(
            JsonApiRelationships.class
          );
          if (relationshipsAnnotation != null) {
            Object relationship = relationships.get(
              relationshipsAnnotation.value()
            );
            try {
                  ...
                  if (Collection.class.isAssignableFrom(field.getType())) {
                    ...
                    if (data instanceof List) {
                      ...
                      throw new IllegalArgumentException(
                        CANNOT_DESERIALIZE_INPUT_TO_ENTITY_MODEL
                      );
                    }
  ...
  private EntityModel<?> createRelationship(
    ...
    return (EntityModel<?>) convertToResource(
      entry,
      true,
      doc,
  ...
  protected @Nullable Map<
    String,
    Object
  > findIncludedAttributesForRelationshipObject(
    String id,
    String type,
    JsonApiDocument doc
  ) {
    for (JsonApiData jsonApiData : doc.getIncluded()) {
      if (
        id.equals(jsonApiData.getId()) && type.equals(jsonApiData.getType())
      ) {
  ...
  protected @Nullable Map<
    String,
    Object
  > findIncludedRelationshipsForRelationshipObject(
    String id,
    String type,
    JsonApiDocument doc
  ) {
